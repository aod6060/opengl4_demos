...
private:
	...
	// Enviormental Map
	Shader envmapVertexShader;
	Shader envmapFragmentShader;
	Program envmapProgram;
	// Textures
	Material dirt;
	Material grass;
	Material lava;
...
public:
	...
	virtual void init() {
		...
		// Shader
		vertexShader.init(GL_VERTEX_SHADER, "data/shaders/envmap_refract_normal_map/shader.vert");
		fragmentShader.init(GL_FRAGMENT_SHADER, "data/shaders/envmap_refract_normal_map/shader.frag");
		...
		// Handle Skybox
		// Create Shader
		skyboxVertexShader.init(GL_VERTEX_SHADER, "data/shaders/envmap_refract_normal_map/skybox.vert");
		skyboxFragmentShader.init(GL_FRAGMENT_SHADER, "data/shaders/envmap_refract_normal_map/skybox.frag");
		...
		// Enviormental Mapping
		envmapVertexShader.init(GL_VERTEX_SHADER, "data/shaders/envmap_refract_normal_map/envmap.vert");
		envmapFragmentShader.init(GL_FRAGMENT_SHADER, "data/shaders/envmap_refract_normal_map/envmap.frag");
		envmapProgram.attach(&envmapVertexShader);
		envmapProgram.attach(&envmapFragmentShader);
		envmapProgram.init();
		envmapProgram.bind();
		envmapProgram.create_uniform("projection");
		envmapProgram.create_uniform("view");
		envmapProgram.create_uniform("model");
		envmapProgram.create_uniform("normalModel");
		envmapProgram.create_uniform("cameraPos");
		light.createLightStructure(envmapProgram);
		envmapProgram.create_uniform("skybox");
		envmapProgram.set1i("skybox", 0);
		envmapProgram.create_uniform("normal_map");
		envmapProgram.set1i("normal_map", 1);
		envmapProgram.unbind();
		dirt_normal.init("data/textures/normal_4.png", GL_LINEAR_MIPMAP_LINEAR, GL_LINEAR, true);
	}
	...
	virtual void render() {
		...
		program.unbind();
		// Enviormental Renderer
		envmapProgram.bind();
		envmapProgram.setMat4f("projection", projection);
		envmapProgram.setMat4f("view", view);
		light.bindLightStructure(envmapProgram);
		envmapProgram.set3f("cameraPos", camera.getPos());
		glm::mat4 model = glm::translate(glm::vec3(0.0f, 3.0f, -10.0f)) * glm::rotate(yrot, glm::vec3(0.0f, 1.0f, 0.0f));
		glm::mat4 normalMode = glm::transpose(glm::inverse(model));
		envmapProgram.setMat4f("model", model);
		envmapProgram.setMat4f("normalModel", normalMode);
		skybox.bind();
		dirt_normal.bind(GL_TEXTURE1);
		monkey.render();
		dirt_normal.unbind(GL_TEXTURE1);
		skybox.unbind();
		envmapProgram.unbind();
		// Skybox
...
int main(int argc, char** argv) {
	MaterialApp app;
	app_init("Cubemap: Enviormental Mappinging, Reflection", 1366, 768, &app);
	app_update();
	app_release();
	return 0;
}