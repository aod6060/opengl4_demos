...
public:
	...
	virtual void init() {
		...
		// Shader
		vertexShader.init(GL_VERTEX_SHADER, "data/shaders/point_light/shader.vert");
		fragmentShader.init(GL_FRAGMENT_SHADER, "data/shaders/point_light/shader.frag");
	...
	virtual void update(float delta) {
		...
		light.position = glm::vec3(glm::cos(yrad), 1.0f, glm::sin(yrad)) * 15.0f;
...
int main(int argc, char** argv) {
	MaterialApp app;
	app_init("Lighting Point Lighting", 1366, 768, &app);
	app_update();
	app_release();
	return 0;
}