...
class LightingDiffuseApp : public IApp {
private:
	// Shader
	Shader vertexShader;
...
public:
	...
	virtual void init() {
		...
		// Shader
		vertexShader.init(GL_VERTEX_SHADER, "data/shaders/lighting_diffuse/shader.vert");
		fragmentShader.init(GL_FRAGMENT_SHADER, "data/shaders/lighting_diffuse/shader.frag");
...
int main(int argc, char** argv) {
	LightingDiffuseApp app;
	app_init("Lighting Diffuse App", 1366, 768, &app);
	app_update();
	app_release();
	return 0;
}