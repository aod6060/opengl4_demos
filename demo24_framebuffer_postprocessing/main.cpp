...
public:
	...
	virtual void init() {
		...
		// Shader
		vertexShader.init(GL_VERTEX_SHADER, "data/shaders/framebuffer_postprocessing/shader.vert");
		fragmentShader.init(GL_FRAGMENT_SHADER, "data/shaders/framebuffer_postprocessing/shader.frag");
		...
		// Framebuffer
		framebuffer.init();
		renderTexture.initEmpty(app_getWidth(), app_getHeight(), GL_RGB, GL_RGB);
		renderTexture.bind();
		...
		this->postprocessVertexShader.init(GL_VERTEX_SHADER, "data/shaders/framebuffer_postprocessing/postprocess.vert");
		this->postprocessFragmentShader.init(GL_FRAGMENT_SHADER, "data/shaders/framebuffer_postprocessing/postprocess.frag");
		...
		this->postprocessProgram.create_uniform("depth0");
		this->postprocessProgram.set1i("depth0", 1);
	...
	void render_scene() {
		...
		screen.render();
		depthTexture.unbind(GL_TEXTURE1);
		renderTexture.unbind();
