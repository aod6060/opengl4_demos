...
void main() {
	...
	vec3 diffuse = albedo * light.diffuse * ndotl * attenuation;
	vec3 ambient = albedo * light.ambient * attenuation;
	
	vec3 halfVec = normalize(view + lightDir);
	float ndoth = max(dot(normal, halfVec), 0.0);
	//float shinny = material.shinny * specular_map.r;
	
	vec3 specular = vec3(light.intencity * specular_map.r) * pow(ndoth, material.shinny) * attenuation;
