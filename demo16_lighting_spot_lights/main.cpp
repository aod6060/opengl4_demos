...
struct Light {
	...
	void createLightStructure(Program& prog) {
		...
		prog.create_uniform("light.constant");
		prog.create_uniform("light.linear");
		prog.create_uniform("light.quad");
		// Spotlight
		prog.create_uniform("light.cutOff");
		prog.create_uniform("light.direction");
		prog.create_uniform("light.outerCutoff");
	}
	...
	void bindLightStructure(Program& prog) {
		...
		prog.set1f("light.constant", this->constant);
		prog.set1f("light.linear", this->linear);
		prog.set1f("light.quad", this->quadratic);
		prog.set1f("light.cutOff", this->cutOff);
		prog.set3f("light.direction", this->direction);
		prog.set1f("light.outerCutoff", this->outerCutOff);
	}
};
...
public:
	...
	virtual void init() {
		...
		// Shader
		vertexShader.init(GL_VERTEX_SHADER, "data/shaders/spot_light/shader.vert");
		fragmentShader.init(GL_FRAGMENT_SHADER, "data/shaders/spot_light/shader.frag");
	...
	virtual void update(float delta) {
		...
		float yrad = glm::radians(yrot);

		light.position = glm::vec3(glm::cos(yrad), 1.0f, glm::sin(yrad)) * 5.0f;
		...
		light.cutOff = glm::cos(glm::radians(12.5));
	...
	virtual void render() {
		...
		this->draw(barrel, dirt, glm::vec3(0.0f, 5.0f, -5.0f));
		this->draw(cube, panel, glm::vec3(0.0f, 1.0f, 5.0f));
		this->draw(death_donut, lava, glm::vec3(-5.0f, 5.5f, 0.0f));
...
int main(int argc, char** argv) {
	MaterialApp app;
	app_init("Lighting Spot Lighting", 1366, 768, &app);
	app_update();
	app_release();
	return 0;
}